...
function gml_program(l_sources)constructor{
	...
	static h_has_fast_getter=function(l_fd){
		return variable_struct_exists(api_api_fast_field_getters,l_fd);
	}
	static h_get_fast_getter=function(l_fd){
		return api_api_fast_field_getters[$ l_fd];
	}
	static h_get_fast_setter=function(l_fd){
		return api_api_fast_field_setters[$ l_fd];
	}
		...
		while(l__g<array_length(l__g1)){
			...
			if(variable_struct_exists(self.h_script_map,l_scr.h_name)){
				if(l_scr.h_name==l_main){
					...
					var l_tmp;
					if(l__g2.__enumIndex__==95)l_tmp=array_length(l__g2.h_nodes)==0; else l_tmp=false;
					if(l_tmp){
	...
	static h_eval=function(l_q){
		...
		switch(l__g.__enumIndex__){
			...
			case 3:l_r=l__g.h_value;break;
			case 55:
				var l__hx_tmp=l__g.h_obj;
				if(l__hx_tmp.__enumIndex__==13){
					var l_d=l__g.h_d;
