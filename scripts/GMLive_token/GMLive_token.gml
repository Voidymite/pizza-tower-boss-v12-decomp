...
function mc_gml_token_sqb_open():mc_gml_token()constructor{
	/// @hint {gml_pos} :h_d
	/// @hint {int} :h_accessorChar
	static __enumParams__=["h_d","h_accessorChar"];
	static __enumIndex__=23;
}

if(live_enabled)
function gml_token_sqb_open(l_h_d,l_h_accessorChar){
	var l_this=new mc_gml_token_sqb_open();
	l_this.h_d=l_h_d;
	l_this.h_accessorChar=l_h_accessorChar;
	return l_this
}
